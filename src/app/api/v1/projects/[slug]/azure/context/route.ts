...
async function armGet(token: string, path: string): Promise<unknown | null> {
  try {
    const res = await fetch(`${ARM}${path}`, { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" });
...
export async function GET(_req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const cp = await prisma.cloudProvider.findFirst({
    ...
    select: { id: true, accountRef: true, resourceGroup: true, region: true, cloudEnvironment: true },
  });
  ...
  if (tok.ok) {
    const subs = (await armGet(tok.accessToken, "/subscriptions?api-version=2020-01-01")) as
      ...
      | null;
    subscriptions = (subs?.value ?? []).map((s) => ({ subscriptionId: s.subscriptionId, displayName: s.displayName, state: s.state }));
    const rgs = (await armGet(tok.accessToken, `/subscriptions/${cp.accountRef}/resourcegroups?api-version=2021-04-01`)) as
...
export async function PATCH(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  if (parsed.data.resourceGroup !== undefined) data.resourceGroup = parsed.data.resourceGroup || null;
