...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!sess) return NextResponse.json({ ok: false, code: "unauthenticated" }, { status: 401 });
  const { id } = await ctx.params;
  // `?force=1` unlinks the provider from any environments first, then deletes.
  const force = new URL(req.url).searchParams.get("force") === "1";
  const res = await deleteProvider(sess.userId, id, force);
  if (!res.ok) {
    const status = res.code === "in_use" ? 409 : 404;
    const message =
      res.code === "in_use"
        ? `This provider is linked to ${res.inUse ?? "one or more"} environment(s).`
        : "Provider not found.";
    return NextResponse.json({ ok: false, code: res.code, message, inUse: res.inUse }, { status });
  }
  const meta = extractRequestMeta(req);
  await audit({
    userId: sess.userId,
    ...
    ipAddress: meta.ipAddress,
    userAgent: meta.userAgent,
    metadata: { unlinkedEnvs: res.unlinkedEnvs },
  });
  return NextResponse.json({ ok: true, unlinkedEnvs: res.unlinkedEnvs });
}
