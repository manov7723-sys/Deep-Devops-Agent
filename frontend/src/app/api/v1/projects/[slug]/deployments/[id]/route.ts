...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string; id: string }> }) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { ok: false, message: "action must be 'rollback' or 'redeploy'." },
      { status: 400 },
    );
  ...
  if (!rec)
    return NextResponse.json({ ok: false, message: "Deployment not found." }, { status: 404 });

  if (parsed.data.action === "rollback") {
    const res = await rollbackDeployment(projectId, rec.envKey, rec.appName, {
  ...
  if (!target)
    return NextResponse.json(
      { ok: false, message: `No deployable env "${rec.envKey}" (cluster disconnected?).` },
      { status: 400 },
    );
  ...
  return NextResponse.json({
    ...
    approvalId,
    message: `Redeploy of ${rec.appName} submitted for approval — approve it on the Approvals page.`,
  });
}
