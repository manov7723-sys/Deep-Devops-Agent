...
import { listPods } from "@/lib/observability/cluster-logs";

/** GET /projects/[slug]/envs/[key]/logs/pods?namespace=… — pods in a namespace. */
export async function GET(req: Request, ctx: { params: Promise<{ slug: string; key: string }> }) {
  ...
  const res = await listPods(env.id, namespace);
  if (!res.ok) return NextResponse.json({ ok: false, message: res.error }, { status: 400 });
  return NextResponse.json({ ...res, namespace });
}
