...
async function resolveAzureEnv(projectId: string, key: string) {
  ...
  if (cp?.kind !== "azure")
    return {
      ...
      code: "not_azure",
      status: 400,
      message: "Azure Monitor alarms are AKS/Azure only.",
    };
    ...
    return NextResponse.json(
      {
        ok: false,
        code: "no_cluster",
        message: "Couldn't determine the AKS cluster name. Pass clusterName.",
      },
      { status: 400 },
    );
...
export async function GET(req: Request, ctx: { params: Promise<{ slug: string; key: string }> }) {
  ...
  if (!clusterName)
    return NextResponse.json(
      { ok: false, code: "no_cluster", message: "Couldn't determine the AKS cluster name." },
      { status: 400 },
    );
  ...
  return NextResponse.json({
    ...
    alarms: sync.ok
      ? sync.alarms
      : res.alarms.map((a) => ({ name: a.name, metric: a.metric, state: "OK" as const })),
