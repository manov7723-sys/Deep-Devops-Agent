...
import { setupGkeAlarms, listGkeAlarms, gkeClusterFromEnv, type GcpMetricKey } from "@/lib/cloud/gcp-monitor";
...
async function resolveGcpEnv(projectId: string, key: string) {
  ...
  if (cp?.kind !== "gcp") return { ok: false as const, code: "not_gcp", status: 400, message: "GCP Monitoring alarms are GKE/GCP only." };
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string; key: string }> }) {
  ...
  if (!clusterName) return NextResponse.json({ ok: false, code: "no_cluster", message: "Couldn't determine the GKE cluster name. Pass clusterName." }, { status: 400 });

  const metrics = (parsed.data.metrics ?? [...METRIC_KEYS]) as GcpMetricKey[];
  const result = await setupGkeAlarms({ cloudProviderId: r.env.cloudProviderId, clusterName, email: parsed.data.email, metrics });
...
export async function GET(req: Request, ctx: { params: Promise<{ slug: string; key: string }> }) {
  ...
  const clusterName = new URL(req.url).searchParams.get("clusterName") || (await gkeClusterFromEnv(r.env.id));
  if (!clusterName) return NextResponse.json({ ok: false, code: "no_cluster", message: "Couldn't determine the GKE cluster name." }, { status: 400 });
  ...
  return NextResponse.json({ ok: true, clusterName, configured: res.alarms.length, alarms: res.alarms });
}
