...
const Body = z.object({
  datasetId: z
    .string()
    .trim()
    .regex(/^[A-Za-z0-9_]{1,1024}$/, "letters, numbers, underscores only"),
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  if (!res.ok)
    return NextResponse.json(
      { ok: false, code: "gcp_setup_failed", message: res.error },
      { status: 400 },
    );

  await prisma.cloudProvider.update({
    where: { id: cp.id },
    data: { costDatasetId: parsed.data.datasetId },
  });
  ...
  return NextResponse.json({
    ...
    nextStep:
      "Now enable Billing → BigQuery export to this dataset in the GCP console (the one step with no API). Cost appears within a few hours.",
  });
}
