...
export async function GET(_req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const clouds = rows
    .map((r) => r.kind)
    .filter((k): k is Cloud => k === "aws" || k === "azure" || k === "gcp");
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
    ...
    return NextResponse.json(
      {
        ok: false,
        message:
          "This project has no cloud provider connected. Connect one on the Cloud providers page first.",
      },
      { status: 400 },
    );
  ...
  if (!clouds.includes(cloud))
    return NextResponse.json(
      {
        ok: false,
        message: `This project isn't connected to ${cloud}. Connected: ${clouds.join(", ")}.`,
      },
      { status: 400 },
    );
