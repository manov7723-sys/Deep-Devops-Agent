...
import { getChatOpsStatus, setChatOpsWebhook, setChatOpsEnabled, removeChatOps } from "@/lib/integrations/chatops";
...
const PutBody = z
  ...
  .refine(
    (b) => (b.provider === "slack" ? b.webhookUrl.startsWith("https://hooks.slack.com/") : b.webhookUrl.startsWith("https://")),
    { message: "Teams needs an https webhook URL (…webhook.office.com or Power Automate); Slack needs https://hooks.slack.com/…", path: ["webhookUrl"] },
  );
...
export async function PUT(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  await setChatOpsWebhook(gate.access.project.id, parsed.data.provider, parsed.data.webhookUrl, parsed.data.channel ?? null, parsed.data.enabled);
  return NextResponse.json({ ok: true, ...(await getChatOpsStatus(gate.access.project.id)) });
}
