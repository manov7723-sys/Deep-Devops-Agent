...
export async function GET(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  if (!cp)
    return NextResponse.json({
      ...
      connected: false,
      clusters: [],
      note: "Connect an AWS account on the Cloud providers page first.",
    });
  ...
  if (!creds.ok)
    return NextResponse.json({ ok: true, connected: false, clusters: [], note: creds.message });
