...
export async function GET(_req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const monitors = await prisma.uptimeMonitor.findMany({
    ...
    include: { checks: { orderBy: { at: "desc" }, take: 20, select: { at: true, ok: true, latencyMs: true, status: true } } },
  });
...
const Body = z.object({
  ...
  url: z.string().trim().url().refine((u) => /^https?:\/\//.test(u), "Must be an http(s) URL."),
...
export async function POST(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const m = await prisma.uptimeMonitor.create({ data: { projectId: gate.access.project.id, ...parsed.data } });
