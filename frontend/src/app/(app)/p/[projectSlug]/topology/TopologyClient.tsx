...
type GNode = {
  id: string;
  kind: "ingress" | "service" | "deployment" | "pod";
  label: string;
  sub?: string;
  status: GStatus;
  namespace: string;
...
const KIND_BG: Record<GNode["kind"], string> = {
  ingress: "#eef4ff",
  service: "#f3edff",
  deployment: "#eafaf1",
  pod: "#f6f8fa",
};
const STATUS_BORDER: Record<GStatus, string> = {
  ok: "#30a46c",
  warn: "#f5a623",
  danger: "#e5484d",
  default: "#8b93a7",
...
function toFlow(g: Topo): { nodes: Node[]; edges: Edge[] } {
  ...
  const edges: Edge[] = g.edges.map((e) => ({
    ...
    markerEnd: {
      ...
      color: e.kind === "dependency" ? "#8b5cf6" : "#9aa3b2",
    },
  }));
...
export function TopologyClient({ slug }: { slug: string }) {
  ...
  const q = useQuery<Topo>({
    ...
    queryFn: () =>
      api.get<Topo>(
        `/projects/${slug}/topology?envKey=${encodeURIComponent(envKey)}&namespace=${encodeURIComponent(ns)}`,
      ),
    enabled: !!envKey,
  ...
  return (
    <div className="col gap-5">
      ...
      {q.isError ? (
        <Block>
          <Block.Error message={apiErrorMessage(q.error)} onRetry={() => q.refetch()} />
        </Block>
      ) : q.isLoading ? (
        <Block>
          <Block.Loading />
        ...
        <Block>
          <Block.Empty
            ...
            description="No apps found in this scope. Deploy an app, or pick a different namespace."
          />
        </Block>
      ) : (
        <>
          ...
          <div className="row gap-3 wrap" style={{ fontSize: 11.5, alignItems: "center" }}>
            ...
            <span className="faint">
              — solid = wiring · dashed purple = app dependency (with port)
            </span>
          </div>
          <div
            style={{
