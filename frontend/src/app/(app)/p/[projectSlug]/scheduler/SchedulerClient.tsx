...
export function SchedulerClient({ slug }: { slug: string }) {
  ...
  const envValue =
    envKey ||
    (projectActiveEnv && targets.some((t) => t.envKey === projectActiveEnv)
      ? projectActiveEnv
      : "") ||
    targets[0]?.envKey ||
    "";
  ...
  const create = useMutation({
    mutationFn: () =>
      api.post(`/projects/${slug}/scheduler`, {
        envKey: envValue,
    ...
    <div className="col gap-5">
      ...
      {targets.length === 0 ? (
        <Block>
          <Block.Empty
            icon="globe"
            title="No deployable clusters"
            description="Connect a cluster on the Clusters page first — then you can schedule deployments to it."
          />
        </Block>
      ) : (
        <Block>
          <Block.Header>
            <Block.Title sub="Fill in what to deploy and when. It runs automatically at that time.">
            ...
            <div className="col gap-3">
              <div className="row gap-3 wrap" style={{ alignItems: "flex-end" }}>
                ...
                <div style={{ minWidth: 300, flex: 1 }}>
                  <Field label="Image">
                    <Input
                      value={image}
                      placeholder="registry/my-app:tag"
              ...
              <div className="row gap-3 wrap" style={{ alignItems: "flex-end" }}>
                ...
                <div className="row gap-2" style={{ paddingBottom: 8 }}>
                  <Btn variant="outline" size="sm" onClick={() => quickSet("1h")}>
                    In 1 hour
                  </Btn>
                  <Btn variant="outline" size="sm" onClick={() => quickSet("tonight")}>
                    Tonight 9 PM
                  </Btn>
                  <Btn variant="outline" size="sm" onClick={() => quickSet("tomorrow")}>
                    Tomorrow 9 AM
                  </Btn>
                </div>
                <Btn
      ...
      {scheduled.length === 0 ? (
        <Block>
          <Block.Empty
            ...
            description="Scheduled deployments will appear here."
          />
        </Block>
      ) : (
        <div className="col gap-3">
        ...
        <div className="row between wrap" style={{ alignItems: "center", gap: 10 }}>
          <div className="col" style={{ gap: 2, minWidth: 0 }}>
            <span className="row gap-2" style={{ alignItems: "center" }}>
              ...
              <Badge tone={STATUS_TONE[s.status] ?? "default"} withDot>
                {s.status}
              </Badge>
              {s.status === "pending" &&
                (s.approved ? (
          ...
          <div className="row gap-3 wrap" style={{ alignItems: "center" }}>
            ...
            {s.status === "pending" && (
              <Btn variant="ghost" size="sm" icon="trash" loading={cancelling} onClick={onCancel}>
                Cancel
              </Btn>
            )}
          </div>
