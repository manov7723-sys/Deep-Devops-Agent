...
export function DeploymentsClient({ slug }: { slug: string }) {
  ...
  const act = useMutation({
    mutationFn: (v: { id: string; action: "rollback" | "redeploy" }) =>
      api.post<{ ok: boolean; message: string }>(`/projects/${slug}/deployments/${v.id}`, {
        action: v.action,
      }),
    onMutate: (v) => {
      setNote(null);
      ...
      {deployments.length === 0 ? (
        <Block>
          <Block.Empty
            ...
            description="Deploys will show up here — from the Scheduler, the deploy wizard, or the agent."
          />
        </Block>
      ) : (
                ...
                <div className="row between wrap" style={{ alignItems: "center", gap: 10 }}>
                  <div className="col" style={{ gap: 2, minWidth: 0 }}>
                    <span className="row gap-2" style={{ alignItems: "center" }}>
                      ...
                      <Badge tone={STATUS_TONE[d.status] ?? "default"} withDot>
                  ...
                  <div className="row gap-3 wrap" style={{ alignItems: "center" }}>
                    ...
                    <Btn
                      ...
                      size="sm"
                      icon="clock"
                      loading={busyId === d.id && act.variables?.action === "rollback"}
                      disabled={busyId != null}
                      onClick={() => act.mutate({ id: d.id, action: "rollback" })}
                    >
                      Rollback
                    </Btn>
                    <Btn
                      ...
                      size="sm"
                      icon="refresh"
                      loading={busyId === d.id && act.variables?.action === "redeploy"}
                      disabled={busyId != null}
                      onClick={() => act.mutate({ id: d.id, action: "redeploy" })}
                    >
                      Redeploy
                    </Btn>
                  </div>
