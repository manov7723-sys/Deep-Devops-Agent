...
export function UptimeClient({ slug }: { slug: string }) {
  ...
  const add = useMutation({
    mutationFn: () =>
      api.post(`/projects/${slug}/uptime`, {
        name: name.trim(),
        url: url.trim(),
        expectedStatus: Number(expected) || 200,
        intervalSec: Number(interval) || 300,
      }),
  ...
  const toggle = useMutation({
    mutationFn: (v: { id: string; enabled: boolean }) =>
      api.patch(`/projects/${slug}/uptime/${v.id}`, { enabled: v.enabled }),
  ...
  return (
    <div className="col gap-5">
      ...
      <Block>
        <Block.Header>
          <Block.Title sub="Add a URL to monitor.">New monitor</Block.Title>
        </Block.Header>
        <Block.Body>
      ...
      {monitors.length === 0 ? (
        <Block>
          <Block.Empty
            ...
            description="Add a URL above to start watching it."
          />
        </Block>
      ) : (
        <div className="col gap-3">
          {monitors.map((m) => (
            <MonitorRow
              key={m.id}
              m={m}
              onToggle={(v) => toggle.mutate({ id: m.id, enabled: v })}
          ...
          <div className="row between wrap" style={{ alignItems: "center", gap: 10 }}>
            ...
            <div className="row gap-3 wrap" style={{ alignItems: "center" }}>
              <span className="muted" style={{ fontSize: 12 }}>
                ...
                {m.lastCheckedAt
                  ? ` · ${new Date(m.lastCheckedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`
                  : " · not checked"}
              </span>
              ...
              {checks.map((c, i) => (
                <span
                  ...
                  style={{
                    ...
                    height: 20,
                    borderRadius: 2,
                    background: c.ok ? "var(--ok, #30a46c)" : "var(--danger, #e5484d)",
                  }}
                />
              ))}
            </div>
