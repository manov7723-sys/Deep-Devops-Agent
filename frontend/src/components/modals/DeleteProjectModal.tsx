    ...
    <Modal
      ...
      footer={
        <div className="row between" style={{ gap: 8 }}>
          <Btn variant="ghost" onClick={() => close(false)} disabled={del.isPending}>Cancel</Btn>
          <Btn variant="danger" icon="trash" disabled={!match || del.isPending} loading={del.isPending} onClick={() => del.mutate()}>
            Delete project
          </Btn>
        </div>
      }
    >
      {project && (
        <div className="col gap-3">
          ...
          >
            ⚠️ This removes the project and its settings, environments, deployments, alerts and history from your
            workspace. Connected clusters and cloud accounts are <b>not</b> deleted — only this project. This can’t be
            easily undone.
          </div>
          <Field label={`Type the project name to confirm`}>
            <Input value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder={name} autoFocus
              onKeyDown={(e) => { if (e.key === "Enter" && match) del.mutate(); }} />
          </Field>
