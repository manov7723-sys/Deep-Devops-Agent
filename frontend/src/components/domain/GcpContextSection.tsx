        ...
        {isLoading ? (
          ...
          <div className="col gap-4" style={{ maxWidth: 560 }}>
            ...
            <Field label="GCP project" hint="Which Google Cloud project to work in.">
              {projOpts.length > 0 ? (
                <Select value={proj} onValueChange={(v) => { setProj(v); setDirty(true); }} options={projOpts} placeholder="Pick a GCP project" />
              ) : (
                <div className="faint" style={{ fontSize: 12 }}>No projects visible to this account.</div>
              )}
            </Field>
