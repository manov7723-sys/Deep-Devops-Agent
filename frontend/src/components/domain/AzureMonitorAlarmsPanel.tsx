...
/**
 * Azure Monitor metric alerts for the env's AKS cluster — the Azure counterpart
 * of the CloudWatch panel. Node CPU/memory/disk % thresholds → email action
 * group + the Alerts tab. Only renders for Azure envs.
 *
 * Backed by POST/GET /projects/[slug]/envs/[key]/azure-monitor/alarms.
 */
...
export function AzureMonitorAlarmsPanel({ slug, env }: { slug: string; env: EnvFilterValue }) {
  ...
  const [selected, setSelected] = useState<Set<MetricKey>>(new Set(["cpu", "memory", "disk"]));
  const [result, setResult] = useState<SetupResult | null>(null);
  const [error, setError] = useState<string | null>(null);

  const existing = useQuery<{ ok: boolean; clusterName?: string; configured?: number }>({
    queryKey: ["p", slug, "azure-alarms", activeKey],
    queryFn: () => api.get(`/projects/${slug}/envs/${activeKey}/azure-monitor/alarms`),
    enabled: !!activeKey && activeEnv?.cloudKind === "azure",
    retry: false,
    staleTime: 20_000,
  });
  ...
  const setup = useMutation({
    mutationFn: () =>
      api.post<SetupResult>(`/projects/${slug}/envs/${activeKey}/azure-monitor/alarms`, {
  ...
  // Only relevant for Azure environments.
  if (!activeKey || activeEnv?.cloudKind !== "azure") return null;
