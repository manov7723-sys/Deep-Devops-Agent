...
type NodeDetail = { ok: true; cpuPct?: number; memPct?: number; schedulable: boolean; pods: Array<{ name: string; namespace: string; status: string }> };
...
function NodeActions({ slug, envKey, node }: { slug: string; envKey: string; node: string }) {
  ...
  const detailQ = useQuery<NodeDetail>({
    ...
    queryFn: () => api.get<NodeDetail>(`/projects/${slug}/cloud/node-detail?envKey=${encodeURIComponent(envKey)}&node=${encodeURIComponent(node)}`),
    enabled: open,
  });

  const act = useMutation({
    mutationFn: (action: "cordon" | "uncordon" | "drain") =>
      api.post<{ ok: boolean; message: string }>(`/projects/${slug}/cloud/node-action`, { envKey, node, action }),
    onMutate: () => { setMsg(null); setErr(null); },
    onSuccess: (r) => { setMsg(r.message); qc.invalidateQueries({ queryKey: ["p", slug, "node-detail", envKey, node] }); },
  ...
  return (
    <div className="col gap-2">
      <div className="row gap-2 wrap">
        ...
        {d?.schedulable === false ? (
          <Btn size="sm" variant="ghost" icon="check" loading={act.isPending} onClick={() => act.mutate("uncordon")}>
            Uncordon
          </Btn>
        ) : (
          <Btn size="sm" variant="ghost" icon="pause" loading={act.isPending} onClick={() => act.mutate("cordon")}>
            Cordon
          </Btn>
        )}
        <Btn
          ...
          onClick={() => { if (confirm(`Drain node "${node}"? This evicts its pods (they reschedule elsewhere).`)) act.mutate("drain"); }}
        >
          Drain
        </Btn>
      </div>
      ...
      {open && (
        <div className="col gap-2" style={{ borderTop: "1px solid var(--border)", paddingTop: 8 }}>
          {detailQ.isLoading ? (
            <span className="muted" style={{ fontSize: 12 }}>Loading node metrics + pods…</span>
          ) : !d ? (
            <span className="muted" style={{ fontSize: 12 }}>Couldn&apos;t load node details.</span>
          ) : (
            <>
              <div className="row gap-2" style={{ alignItems: "center" }}>
                {d.schedulable ? <Badge tone="ok">schedulable</Badge> : <Badge tone="warn">cordoned</Badge>}
                <span className="muted" style={{ fontSize: 11.5 }}>{d.pods.length} pod{d.pods.length === 1 ? "" : "s"}</span>
              </div>
              {(d.cpuPct != null || d.memPct != null) ? (
                ...
                <span className="muted" style={{ fontSize: 11 }}>Live CPU/memory need metrics-server on the cluster.</span>
              )}
              {d.pods.length > 0 && (
                <div style={{ maxHeight: 160, overflow: "auto", fontSize: 11.5 }}>
                  {d.pods.map((p) => (
                    <div key={`${p.namespace}/${p.name}`} className="row between" style={{ padding: "2px 0" }}>
                      <span className="mono" style={{ minWidth: 0, overflow: "hidden", textOverflow: "ellipsis" }}>{p.namespace}/{p.name}</span>
