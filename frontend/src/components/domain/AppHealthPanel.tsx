...
export function AppHealthPanel({
  ...
  return (
    <Block>
      <Block.Header>
        <Block.Title sub={`Are your apps running in "${namespace}"? Updated every 15s.`}>
          Application health
        </Block.Title>
        <Block.Actions>
          {apps.length > 0 && (
            <StatusDot
              tone={allUp ? "ok" : apps.some((a) => a.status === "down") ? "danger" : "warn"}
              label={allUp ? "all healthy" : "needs attention"}
            />
          )}
        </Block.Actions>
      </Block.Header>
      <Block.Body>
        {isLoading ? (
          ...
          <span className="muted" style={{ fontSize: 13 }}>
            No apps found in “{namespace}”. Deploy an app to this namespace to see its health here.
          </span>
        ) : (
          <div className="col gap-2">
            {apps.map((a) => (
              <div
                key={`${a.kind}/${a.name}`}
                className="row between card card-pad"
