...
type StackField = { key: string; type: "string" | "number"; description: string; default?: string | number; options?: string[] };
...
function sanitize(raw: string): string {
  return (raw || "app").toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 63) || "app";
}
...
export function CicdSetupBox({ slug }: { slug: string }) {
  ...
  const branchesQ = useQuery<{ ok: boolean; branches: string[]; defaultBranch?: string }>({
    ...
    queryFn: () => api.get<{ ok: boolean; branches: string[]; defaultBranch?: string }>(`/projects/${slug}/cicd/branches`, { repo: repoFullName }),
  ...
  useEffect(() => {
    ...
    setDockerParams((prev) => {
      const next = { ...prev };
      let changed = false;
      for (const f of stack.fields) if (next[f.key] === undefined) { next[f.key] = String(f.default ?? ""); changed = true; }
  ...
  return (
    <Block>
      ...
      <Block.Body>
        {opts.isLoading ? (
          <span className="muted" style={{ fontSize: 13 }}>Loading your repos, envs and cloud…</span>
        ) : !data ? (
          <span style={{ fontSize: 13, color: "var(--danger)" }}>Couldn&apos;t load setup options.</span>
        ) : result ? (
          <ResultPanel result={result} onReset={() => { setup.reset(); }} />
        ) : (
          <div className="col gap-4" style={{ maxWidth: 620 }}>
            {!data.registrySupported && (
              <div style={{ fontSize: 12.5, color: "var(--danger)", border: "1px dashed var(--border)", borderRadius: 8, padding: 10 }}>
                Automatic CI/CD targets AWS ECR today. Connected: {data.clouds.join(", ") || "none"}. Connect an AWS account on the Cloud tab first.
              </div>
            )}
            ...
            <Field label="Image name" hint="The container image (ECR repo) name. Used in BOTH the CI push and the Deployment.">
              <Input value={imageName} placeholder="dynamic-react-app" onChange={(e) => setImageName(e.target.value)} />
            </Field>
            ...
            <Field label="Trigger branch" hint="CI runs on push to this branch. Loaded from the repo.">
              {branchList.length ? (
                <Select value={branch} onValueChange={setBranch} ariaLabel="Trigger branch" options={branchList.map((b) => ({ value: b, label: b }))} />
              ) : (
                <Input value={branch} placeholder="master" onChange={(e) => setBranch(e.target.value)} />
              )}
            </Field>
            <Field label="Trivy scan gate" hint="Fail the build on HIGH/CRITICAL vulnerabilities before pushing.">
              <Toggle checked={scanGate} onCheckedChange={setScanGate} ariaLabel="Trivy scan gate" />
            </Field>
            <Field label="Agent auto-review" hint="Track it on the CI/CD tab; a failed run started from there gets its workflow auto-fixed + re-run by the agent (up to 3×, commits the fix to the default branch).">
              <Toggle checked={agentReview} onCheckedChange={setAgentReview} ariaLabel="Agent auto-review" />
            </Field>
            ...
            <Field label="Environment" hint="Its cluster + namespace is the deploy target.">
              <Select
                ...
                options={data.envs.map((e) => ({ value: e.envKey, label: `${e.name} · ${e.namespace}${e.cloudKind ? ` · ${e.cloudKind}` : ""}` }))}
              />
            </Field>
            <Field label="App name" hint="Kubernetes Deployment/Service name (lowercase DNS label).">
              <Input value={appName} placeholder="dynamic-react-app" onChange={(e) => setAppName(e.target.value)} />
            </Field>
            ...
            <Field label="Environment variables" hint="One KEY=VALUE per line (optional).">
              <Textarea rows={3} value={envText} placeholder={"REACT_APP_API_URL=https://api.example.com"} onChange={(e) => setEnvText(e.target.value)} />
            </Field>

            {/* ── Files to write (toggle any off) ───────────────── */}
            <SectionLabel>Files to write</SectionLabel>
            <div className="col gap-2">
              <FileToggle label="Dockerfile" hint="+ .dockerignore" on={include.dockerfile} set={(v) => setInc("dockerfile", v)} />
              <FileToggle label="docker-compose.yml" hint="For local runs" on={include.compose} set={(v) => setInc("compose", v)} />
              {stackId === "static-spa" && (
                <FileToggle label="nginx.conf" hint="SPA server config (static-spa only)" on={include.nginx} set={(v) => setInc("nginx", v)} />
              )}
              <FileToggle label="CI workflow" hint="build-and-push.yml — build → Trivy → push to ECR" on={include.ciWorkflow} set={(v) => setInc("ciWorkflow", v)} />
              <FileToggle label="Kubernetes manifest" hint="manifest.yaml — Deployment + Service (+ Ingress)" on={include.manifest} set={(v) => setInc("manifest", v)} />
              <FileToggle label="CD workflow" hint="deploy.yml — deploys after CI succeeds" on={include.cdWorkflow} set={(v) => setInc("cdWorkflow", v)} />
            </div>
            ...
            <div className="row between" style={{ alignItems: "center", borderTop: "1px solid var(--border)", paddingTop: 12 }}>
              <span className="faint" style={{ fontSize: 11.5 }}>Opens a pull request with all files.</span>
              <Btn variant="primary" icon="github" loading={setup.isPending} disabled={!canSubmit} onClick={() => setup.mutate()}>
                Generate &amp; open PR
              </Btn>
            </div>
          </div>
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <span className="muted" style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.4 }}>{children}</span>;
}

function FileToggle({ label, hint, on, set }: { label: string; hint: string; on: boolean; set: (v: boolean) => void }) {
  return (
...
function ResultPanel({ result, onReset }: { result: SetupResult; onReset: () => void }) {
  return (
    <div className="col gap-3" style={{ maxWidth: 620 }}>
      ...
      {result.pullRequest ? (
        <a className="btn primary sm" style={{ width: "fit-content", textDecoration: "none" }} href={result.pullRequest.url} target="_blank" rel="noreferrer">
          <Icon name="github" size={14} /> Review PR #{result.pullRequest.number}
        </a>
      ) : (
        <span className="muted" style={{ fontSize: 13 }}>Files committed to branch <code>{result.branch}</code>.</span>
      )}
      <div className="col gap-1" style={{ border: "1px solid var(--border)", borderRadius: 8, padding: 12, fontSize: 12.5 }}>
        {result.files.map((f) => (
      ...
      <div className="col gap-1" style={{ fontSize: 12.5 }}>
        <span>Image: <span className="mono">{result.imageRef}</span></span>
        <span>Namespace: <span className="mono">{result.namespace}</span></span>
        <span>
          Cluster secret <code>KUBECONFIG_B64</code>:{" "}
          {result.kubeconfigSet ? "set ✅" : `not set — ${result.kubeconfigNote ?? "set it manually so CD can reach the cluster."}`}
        </span>
        {result.registeredPipeline && (
          <span>
            Agent auto-review: <strong>on</strong> — tracked on the CI/CD tab as “{result.registeredPipeline.name}”. Run it from there so a failed run gets auto-fixed.
          </span>
        )}
      </div>
      <div style={{ fontSize: 12.5, color: "var(--muted)" }}>
        Merge the PR into <code>{result.branch}</code> → CI builds, scans &amp; pushes the image → CD deploys it to the cluster after CI succeeds.
      </div>
      <Btn variant="ghost" size="sm" icon="refresh" onClick={onReset}>Set up another</Btn>
    </div>
  );
}
