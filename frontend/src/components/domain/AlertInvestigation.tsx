...
const RISK_TONE: Record<RemediationStep["risk"], BadgeTone> = {
  low: "info",
  medium: "warn",
  high: "danger",
};
const CONF_TONE: Record<Diagnosis["confidence"], BadgeTone> = {
  low: "warn",
  medium: "info",
  high: "ok",
};
...
export function AlertInvestigation({
  ...
  const queued =
    triage.data?.approvalsCreated ?? (d ? d.remediation.filter((r) => r.needsApproval).length : 0);
      ...
      <div className="row gap-2 wrap" style={{ alignItems: "center", paddingLeft: 4 }}>
        ...
        {triage.data && (
          <span className="muted" style={{ fontSize: 11.5 }}>
            Inspected:{" "}
            {triage.data.toolsUsed.length ? [...new Set(triage.data.toolsUsed)].join(", ") : "—"}
          </span>
        )}
      </div>

      {err && (
        <span style={{ color: "var(--danger, #e5484d)", fontSize: 12.5, paddingLeft: 4 }}>
          ❌ {err}
        </span>
      )}
      ...
      {d && (
        ...
        >
          ...
          {queued > 0 && (
            <div
              className="row gap-2"
              style={{ alignItems: "center", borderTop: "1px solid var(--border)", paddingTop: 10 }}
            >
              ...
              <Link
                href={`/p/${slug}/approvals` as Route}
                className="dda-alert-link"
                style={{ fontSize: 12.5 }}
              >
                Review in Approvals →
              </Link>
