 ...
 */
import { ClusterChat, parseListRows, type ClusterChatConfig, type Step, type StepCtx } from "@/components/domain/cluster-chat-engine";
...
type AwsConnectedRoleSource = { connected?: boolean; roleArn?: string | null; providerName?: string | null };
...
const STEPS: Step[] = [
  ...
  {
    page: 1, kind: "select", key: "region", label: "Region",
    options: () => AWS_REGIONS.map((r) => ({ value: r, label: r })),
    default: () => "us-east-1",
  },
  {
    page: 1, kind: "choice", key: "createVpc", label: "Networking",
    choices: [{ value: true, label: "Create a new VPC" }, { value: false, label: "Reuse an existing VPC" }],
  },
  {
    page: 1, kind: "select", key: "existingVpcId", label: "Existing VPC",
    hint: "VPCs in the selected environment's account & region.",
    emptyNote: "No VPCs found for this env/region (or AWS isn't reachable). Switch back to “Create a new VPC”, or check the env's credentials.",
  ...
  {
    page: 1, kind: "multiselect", key: "existingSubnetIds", label: "Cluster subnets", optional: true,
    ...
    options: (c) => {
      ...
      return (src?.subnets ?? [])
        ...
        .map((s) => ({ value: s.subnetId, label: `${s.name ? `${s.name} · ` : ""}${s.subnetId} · ${s.az} · ${s.cidr}` }));
    },
  },
  {
    page: 1, kind: "multiselect", key: "nodeSubnetIds", label: "Node subnets", optional: true,
  ...
  {
    page: 2, kind: "select", key: "kubernetesVersion", label: "Kubernetes version",
    options: (c) => strList(c, "kubernetesVersions", ["1.30"]).map((v) => ({ value: v, label: v })),
  },
  {
    page: 2, kind: "select", key: "instanceType", label: "Node instance type",
    options: (c) => strList(c, "instanceTypes", ["t3.medium"]).map((t) => ({ value: t, label: t })),
  },
  {
    page: 2, kind: "number", key: "desiredNodes", label: "Desired nodes",
    default: () => "2", validate: (v) => (Number(v) >= 1 ? null : "At least 1 node."),
  },
  {
    page: 2, kind: "number", key: "minNodes", label: "Min nodes",
    default: () => "1",
    validate: (v, a) => (Number(v) >= 1 && Number(v) <= Number(a.desiredNodes) ? null : "Min must be ≥ 1 and ≤ desired."),
  },
  {
    page: 2, kind: "number", key: "maxNodes", label: "Max nodes",
    default: () => "3", validate: (v, a) => (Number(v) >= Number(a.desiredNodes) ? null : "Max must be ≥ desired."),
  },
  {
    page: 2, kind: "choice", key: "endpointPublic", label: "API endpoint",
    hint: "Whether the Kubernetes API server is reachable from the internet.",
    choices: [{ value: true, label: "Public endpoint" }, { value: false, label: "Private only" }],
  },
  {
    page: 2, kind: "text", key: "publicAccessCidrs", label: "Public access CIDRs", mono: true,
    hint: "Restrict the public endpoint to these CIDRs (comma-separated). 0.0.0.0/0 = open to all (not recommended for prod).",
    placeholder: "1.2.3.4/32, 10.0.0.0/8", default: () => "0.0.0.0/0",
    skip: (a) => a.endpointPublic === false,
  },
  // ── Page 3 · Security & tags ─────────────────────────────────────────
  {
    page: 3, kind: "text", key: "environment", label: "Environment tag", placeholder: "production", default: () => "production",
  },
  { page: 3, kind: "text", key: "team", label: "Team tag", placeholder: "devops", default: () => "devops" },
  { page: 3, kind: "text", key: "costCenter", label: "Cost center tag", optional: true, placeholder: "CC-1234" },
  {
    page: 3, kind: "choice", key: "controlPlaneLogs", label: "Control-plane logging",
  ...
  {
    page: 3, kind: "choice", key: "secretsEncryption", label: "Secrets encryption (KMS)",
    hint: "Encrypts Kubernetes secrets at rest with a dedicated KMS key.",
    choices: [{ value: true, label: "Enabled (recommended)" }, { value: false, label: "Disabled" }],
  },
  {
    page: 3, kind: "choice", key: "ebsCsi", label: "EBS CSI driver add-on",
    hint: "Enables dynamic persistent volumes (EBS).",
    choices: [{ value: true, label: "Enabled" }, { value: false, label: "Disabled" }],
  },
  // ── Page 4 · Node groups ─────────────────────────────────────────────
  {
    page: 4, kind: "select", key: "systemDiskSize", label: "System node disk (GB)",
    options: (c) => (strList(c, "diskSizes", ["50", "100", "150", "200"]) as unknown[]).map((d) => ({ value: String(d), label: `${d} GB` })),
    default: () => "100",
  },
  {
    page: 4, kind: "choice", key: "appNodeGroup", label: "Application node group",
    hint: "Add a second autoscaling node group for app workloads (system group gets tainted for critical add-ons).",
    choices: [{ value: true, label: "Add app node group" }, { value: false, label: "System group only" }],
  },
  {
    page: 4, kind: "multiselect", key: "appInstanceTypes", label: "App node instance types",
  ...
  {
    page: 4, kind: "choice", key: "appCapacityType", label: "App capacity type",
    skip: (a) => a.appNodeGroup !== true,
    choices: [{ value: "SPOT", label: "Spot (cost-saving)" }, { value: "ON_DEMAND", label: "On-Demand" }],
  },
  {
    page: 4, kind: "number", key: "appMinNodes", label: "App min nodes", default: () => "2",
    skip: (a) => a.appNodeGroup !== true,
  },
  {
    page: 4, kind: "number", key: "appMaxNodes", label: "App max nodes", default: () => "20",
    skip: (a) => a.appNodeGroup !== true,
    validate: (v, a) => (Number(v) >= Number(a.appMinNodes ?? 1) ? null : "Max must be ≥ min."),
  },
  {
    page: 4, kind: "number", key: "appDesiredNodes", label: "App desired nodes", default: () => "3",
    skip: (a) => a.appNodeGroup !== true,
  },
  // ── Page 5 · Access ───────────────────────────────────────────────────
  {
    page: 5, kind: "info", key: "connectedRoleInfo", label: "Already has access",
    text: (c) => {
      const src = c.sources?.connectedRole as AwsConnectedRoleSource | undefined;
      if (!src?.connected) return "Deep Agent's connected AWS account will get cluster-admin automatically once the cluster is created — no action needed.";
      const who = src.roleArn ? src.roleArn : src.providerName ? `${src.providerName} (stored keys)` : "your connected AWS account";
      return `Deep Agent already gets cluster-admin automatically via ${who} — no need to add it below.`;
    },
  },
  {
    page: 5, kind: "list", key: "accessEntries", label: "Additional users/roles", optional: true,
    hint: "Grant other IAM users or roles direct cluster access — e.g. your own AWS user, a teammate, or a CI role.",
    addLabel: "+ Add user",
    max: 10,
    fields: [
      {
        key: "principalArn", label: "IAM user/role ARN", kind: "text", mono: true,
        placeholder: "arn:aws:iam::123456789012:role/devops",
        validate: (v) => (!v || ARN_RE.test(v) ? null : "Must be an IAM user/role ARN, e.g. arn:aws:iam::123456789012:role/devops."),
      },
      {
        key: "policy", label: "Access level", kind: "select",
  ...
  // ── Page 6 · Repository ───────────────────────────────────────────────
  {
    page: 6, kind: "select", key: "repoFullName", label: "GitHub repository",
  ...
  {
    page: 6, kind: "text", key: "ghPath", label: "GitHub file path (folder)",
...
const EKS_CONFIG: ClusterChatConfig = {
  ...
  branchPrefix: "eks",
  applyEta: "~15–20 min",
  pageTitles: ["Networking", "Cluster basics", "Security & tags", "Node groups", "Access", "Repository"],
  ...
  buildBody: (a) => ({
    ...
    existingSubnetIds:
      a.createVpc === false && String(a.existingSubnetIds ?? "").trim()
        ? String(a.existingSubnetIds).split(",").map((s) => s.trim()).filter(Boolean)
        : undefined,
    nodeSubnetIds:
      a.createVpc === false && String(a.nodeSubnetIds ?? "").trim()
        ? String(a.nodeSubnetIds).split(",").map((s) => s.trim()).filter(Boolean)
        : undefined,
    // Production options.
    ...
    publicAccessCidrs: a.endpointPublic !== false ? String(a.publicAccessCidrs ?? "0.0.0.0/0").trim() : undefined,
    ...
    appInstanceTypes:
      a.appNodeGroup === true && String(a.appInstanceTypes ?? "").trim()
        ? String(a.appInstanceTypes).split(",").map((s) => s.trim()).filter(Boolean)
        : undefined,
    appCapacityType: a.appNodeGroup === true ? (a.appCapacityType === "ON_DEMAND" ? "ON_DEMAND" : "SPOT") : undefined,
    ...
    accessEntries: (() => {
      const entries = parseListRows(a.accessEntries)
        .map((r) => ({ principalArn: String(r.principalArn ?? "").trim(), policy: String(r.policy ?? "AmazonEKSClusterAdminPolicy") }))
