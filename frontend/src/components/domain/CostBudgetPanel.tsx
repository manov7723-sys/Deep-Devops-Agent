...
const fmt = (cents: number, cur = "USD") =>
  new Intl.NumberFormat(undefined, { style: "currency", currency: cur || "USD" }).format(
    cents / 100,
  );
...
export function CostBudgetPanel({ slug }: { slug: string }) {
  ...
  const gcpSetup = useMutation({
    mutationFn: () =>
      api.post<{ ok: boolean; nextStep: string }>(`/projects/${slug}/cost/gcp-setup`, {
  ...
  const gcpVerify = useMutation({
    mutationFn: () =>
      api.post<{ ok: boolean; stage: string; message: string }>(
        `/projects/${slug}/cost/gcp-verify`,
        {},
      ),
    onMutate: () => {
      setErr(null);
  ...
  const setBudget = useMutation({
    mutationFn: () =>
      api.post<{ ok: boolean; budgetCents: number }>(`/projects/${slug}/cost/budget`, {
        budgetDollars: Number(budgetInput),
      }),
    onMutate: () => setErr(null),
    onSuccess: () => {
      setBudgetInput("");
  ...
  const pct =
    data && data.budgetCents && data.budgetCents > 0
      ? Math.round((data.projectCents / data.budgetCents) * 100)
      : null;
      ...
      <Block>
        ...
        <Block.Body>
          {data ? (
            ...
            >
              <Stat
                label="Account · month-to-date"
                value={fmt(data.accountCents, cur)}
                icon="dollar"
              />
            </div>
          ) : (
            <span className="muted" style={{ fontSize: 13 }}>
              Click “Refresh cost” to pull live spend.
            </span>
          )}
        </Block.Body>
      </Block>
        ...
        <Block.Header>
          <Block.Title sub="This project's spend vs its budget. Crossing the budget raises an alert (banner + email).">
            <span className="row gap-2" style={{ alignItems: "center" }}>
              ...
              {pct != null && !data?.breached && (
                <Badge tone={pct >= 80 ? "warn" : "ok"}>{pct}% of budget</Badge>
              )}
            </span>
          </Block.Title>
        </Block.Header>
        <Block.Body>
          <div className="col gap-3">
            {data && (
              ...
              >
                <Stat
                  label="Project · month-to-date"
                  value={fmt(data.projectCents, cur)}
                  icon="dollar"
                />
                <Stat
                  label="Forecast (month-end)"
                  value={fmt(data.forecastCents, cur)}
                  icon="stats"
                />
                <Stat
                  label="Budget"
                  value={data.budgetCents != null ? fmt(data.budgetCents, cur) : "—"}
                  icon="flag"
                  sub={pct != null ? `${pct}% used` : "not set"}
                />
              </div>
            )}
            ...
            <div
              className="row gap-2"
              style={{ alignItems: "flex-end", flexWrap: "wrap", maxWidth: 520 }}
            >
              ...
              <Btn
                ...
                disabled={!budgetInput || setBudget.isPending}
            ...
            {err && (
              <span style={{ color: "var(--danger, #e5484d)", fontSize: 12.5 }}>❌ {err}</span>
            )}
              ...
              >
                ...
                >
                  ...
                  <Btn
                    variant="outline"
                    icon="bot"
                    loading={gcpSetup.isPending}
                    disabled={!datasetInput || gcpSetup.isPending}
                    onClick={() => gcpSetup.mutate()}
                  >
                  ...
                  <Btn
                    variant="ghost"
                    icon="refresh"
                    loading={gcpVerify.isPending}
                    disabled={gcpVerify.isPending}
                    onClick={() => gcpVerify.mutate()}
                  >
                    Verify GCP cost
                  </Btn>
                </div>
                  ...
                  <div className="row gap-2" style={{ alignItems: "flex-start" }}>
                    <Badge
                      tone={
                        gcpDiag.stage === "ok"
                          ? "ok"
                          : gcpDiag.stage === "auth" || gcpDiag.stage === "error"
                            ? "danger"
                            : "warn"
                      }
                      withDot
                    >
                      {gcpDiag.stage === "ok"
                        ? "working"
                        : gcpDiag.stage === "no_export"
                          ? "export not on yet"
                          : gcpDiag.stage === "no_dataset"
                            ? "not set up"
                            : gcpDiag.stage}
                    </Badge>
                ...
                <p className="muted" style={{ fontSize: 12 }}>
                  This enables the BigQuery API + creates the dataset automatically. Then you do the
                  one step with no API: in the GCP console →{" "}
                  <b>Billing → Billing export → BigQuery export</b> → point it at this dataset →
                  Save.
                </p>
                {gcpMsg && (
                  <span style={{ fontSize: 12.5, color: "var(--ok, #30a46c)" }}>
                    ✅ Done — {gcpMsg}
                  </span>
                )}
              </div>
            ...
            {data?.breached && (
              <span className="muted" style={{ fontSize: 12 }}>
                Over budget — a high-severity alert was raised. Ask the agent to “analyse cost
