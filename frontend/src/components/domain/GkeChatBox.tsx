...
const STEPS: Step[] = [
  ...
  },
  {
    ...
    emptyNote:
      "No GCP projects visible. Connect GCP (with Resource Manager access) on the Cloud providers page.",
    options: (c) => {
      ...
      if (opts.length === 0 && ctx?.gcpProjectId)
        return [{ value: ctx.gcpProjectId, label: ctx.gcpProjectId }];
      return opts;
    },
    default: (c) => {
      const ctx = c.sources?.gcpProjects as GcpContext | undefined;
  ...
  },
  {
    ...
    kind: "select",
    key: "machineType",
    label: "System node machine type",
    options: (c) =>
      strList(c, "machineTypes", ["n2-standard-4"]).map((t) => ({ value: t, label: t })),
  },
  {
    page: 1,
    kind: "number",
    key: "desiredNodes",
    label: "System nodes per zone",
    default: () => "1",
  ...
  },
  {
    page: 2,
    kind: "choice",
    key: "intranodeVisibility",
    label: "Intranode visibility",
    choices: [
      { value: true, label: "Enabled" },
      { value: false, label: "Disabled" },
    ],
  },
  {
    ...
    choices: [
      { value: true, label: "Private nodes (recommended)" },
      { value: false, label: "Public nodes" },
    ],
  },
  {
    page: 2,
  ...
  // ── Page 3 · Node pools ──────────────────────────────────────────────
  {
    ...
    kind: "select",
    key: "systemDiskType",
    label: "Node disk type",
    options: (c) =>
      strList(c, "diskTypes", ["pd-ssd", "pd-balanced", "pd-standard"]).map((d) => ({
        value: d,
        label: d,
      })),
    default: () => "pd-ssd",
  ...
  // ── Page 4 · Add-ons & tags ──────────────────────────────────────────
  {
    ...
    kind: "choice",
    key: "monitoring",
    label: "Cloud Logging + Managed Prometheus",
    choices: [
      { value: true, label: "Enabled (recommended)" },
      { value: false, label: "Disabled" },
    ],
  },
  {
    page: 4,
    kind: "choice",
    key: "httpLoadBalancing",
    label: "HTTP(S) Load Balancing",
    choices: [
      { value: true, label: "Enabled" },
      { value: false, label: "Disabled" },
    ],
  ...
  },
  {
    ...
    kind: "choice",
    key: "cloudDns",
    label: "Cloud DNS for cluster",
    choices: [
      { value: false, label: "kube-dns (default)" },
      { value: true, label: "Cloud DNS" },
    ],
  },
  {
    page: 4,
  ...
  // ── Page 5 · Networking & repository ─────────────────────────────────
  {
    ...
    choices: [
      ...
      { value: false, label: "Reuse an existing network" },
    ],
  },
  {
    page: 5,
    ...
    emptyNote:
      "No networks found (or GCP isn't reachable). Switch back to “Create a new VPC”, or check the GCP connection.",
  ...
  },
  {
    page: 5,
    kind: "select",
    key: "existingSubnetwork",
    label: "Existing subnetwork",
    optional: true,
...
const GKE_CONFIG: ClusterChatConfig = {
  ...
  blueprintSub:
    "Production GKE blueprint (private, Dataplane V2, Workload Identity, system + app pools, Managed Prometheus). No LLM — runs init → plan → apply.",
  optionsPath: "gke",
  stackPrefix: "gke",
  ...
  extraQueries: [
    { key: "gcpProjects", path: "gcp/context" },
    {
      ...
      path: "gcp/networks",
      params: (a) =>
        a.project ? { project: String(a.project), region: String(a.location ?? "") } : null,
  ...
  buildBody: (a) => ({
    ...
    releaseChannel:
      a.releaseChannel === "STABLE" ? "STABLE" : a.releaseChannel === "RAPID" ? "RAPID" : "REGULAR",
    ...
    systemDiskType:
      a.systemDiskType === "pd-balanced"
        ? "pd-balanced"
        : a.systemDiskType === "pd-standard"
          ? "pd-standard"
          : "pd-ssd",
