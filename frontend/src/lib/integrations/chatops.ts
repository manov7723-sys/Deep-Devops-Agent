...
export async function getChatOpsStatus(projectId: string): Promise<ChatOpsStatus> {
  const row = await prisma.chatOpsWebhook.findUnique({
    where: { projectId },
    select: { enabled: true, channel: true, provider: true },
  });
  return {
    connected: !!row,
    enabled: row?.enabled ?? false,
    provider: (row?.provider as ChatProvider) ?? "teams",
    channel: row?.channel ?? null,
  };
}

export async function setChatOpsWebhook(
  projectId: string,
  provider: ChatProvider,
  webhookUrl: string,
  channel: string | null,
...
async function resolve(projectId: string): Promise<{ url: string; provider: ChatProvider } | null> {
  const row = await prisma.chatOpsWebhook.findUnique({
    where: { projectId },
    select: { webhookRef: true, enabled: true, provider: true },
  });
  if (!row || !row.enabled) return null;
  try {
    return {
      url: decryptSecret(row.webhookRef),
      provider: (row.provider as ChatProvider) ?? "teams",
    };
  } catch {
    return null;
...
function renderPayload(provider: ChatProvider, m: ChatMessage): unknown {
  if (provider === "slack") {
    ...
    return {
      ...
      blocks: [
        ...
        ...(m.text
          ? [{ type: "section", text: { type: "mrkdwn", text: m.text.slice(0, 2800) } }]
          : []),
  ...
  if (m.facts?.length)
    body.push({ type: "FactSet", facts: m.facts.map((f) => ({ title: f.name, value: f.value })) });
  return {
    type: "message",
    attachments: [
      {
...
export async function postToWebhookUrl(
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify(renderPayload(provider, m)),
    });
    if (!res.ok) {
      const t = await res.text().catch(() => "");
      return {
        ok: false,
        error: `Webhook returned HTTP ${res.status}${t ? ` (${t.slice(0, 120)})` : ""}`,
      };
    }
    return { ok: true };
  } catch (e) {
...
export async function postEventToChatOps(
  projectId: string,
  emoji: string,
  title: string,
  detail?: string,
