...
export class GitlabRepoClient implements GitRepoClient {
  ...
  async listFiles(path: string, ref?: string): Promise<GitEntry[]> {
    ...
    return rows.map((e) => ({ path: e.path, name: e.name, type: e.type === "tree" ? "dir" : "file" }));
  }
  ...
  async commitFiles(args: { branch: string; message: string; files: CommitFile[] }): Promise<{ commitSha: string }> {
    ...
    if (!res.ok) throw new Error(`GitLab commit failed: ${res.status} ${await res.text().catch(() => "")}`);
  ...
  async openChangeRequest(args: { sourceBranch: string; targetBranch: string; title: string; body?: string }): Promise<ChangeRequest> {
    ...
    if (!res.ok) throw new Error(`Could not open merge request: ${res.status} ${await res.text().catch(() => "")}`);
