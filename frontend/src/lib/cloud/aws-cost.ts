...
export async function getAwsCost(cloudProviderId: string, now: Date): Promise<CostResult> {
  ...
  if (res.exitCode !== 0) {
    ...
    return { ok: false, error: (res.stderr.trim() || res.stdout.trim()).slice(-300) || "Cost Explorer query failed." };
  }

  try {
    const data = JSON.parse(res.stdout) as { ResultsByTime?: Array<{ Total?: { UnblendedCost?: { Amount?: string; Unit?: string } } }> };
...
export async function getAwsCostByService(cloudProviderId: string, now: Date): Promise<{ ok: true; services: ServiceCost[] } | { ok: false; error: string }> {
  const resolved = await resolveAwsExecEnv(cloudProviderId);
  if (!resolved.ok) return { ok: false, error: resolved.message };
  const start = ymd(new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1)));
  ...
  if (res.exitCode !== 0) return { ok: false, error: (res.stderr.trim() || res.stdout.trim()).slice(-300) || "Cost Explorer query failed." };
  try {
    const data = JSON.parse(res.stdout) as { ResultsByTime?: Array<{ Groups?: Array<{ Keys?: string[]; Metrics?: { UnblendedCost?: { Amount?: string } } }> }> };
    const groups = data.ResultsByTime?.[0]?.Groups ?? [];
    const services = groups
      .map((g) => ({ service: g.Keys?.[0] ?? "Unknown", cents: Math.round((Number(g.Metrics?.UnblendedCost?.Amount ?? "0") || 0) * 100) }))
      .filter((s) => s.cents > 0)
