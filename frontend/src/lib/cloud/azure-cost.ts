...
export type CostResult = { ok: true; totalCents: number; currency: string } | { ok: false; error: string };
 ...
 */
export async function getAzureCost(cloudProviderId: string, resourceGroup?: string): Promise<CostResult> {
  ...
  let res: Response;
  try {
    res = await fetch(`${ARM}${scope}/providers/Microsoft.CostManagement/query?api-version=2023-11-01`, {
      method: "POST",
    ...
    return { ok: false, error: `Network error reaching Azure Cost Management: ${e instanceof Error ? e.message : "error"}` };
  }
  const text = await res.text();
  const data = text ? (JSON.parse(text) as { properties?: { columns?: Array<{ name?: string }>; rows?: unknown[][] }; error?: { message?: string } }) : {};
  if (!res.ok) {
 ...
 */
export async function getAzureClusterCost(cloudProviderId: string, clusterName: string): Promise<CostResult & { resourceGroup?: string }> {
  const tok = await getAzureAccessToken(cloudProviderId);
  if (!tok.ok) return { ok: false, error: tok.error };
  const sub = await subscriptionOf(cloudProviderId);
  if (!sub) return { ok: false, error: "No Azure subscription on the provider." };
  ...
  if (!cluster) return { ok: false, error: `AKS cluster "${clusterName}" not found in the subscription.` };
  ...
  try {
    ...
    if (r.ok) {
      const d = (await r.json().catch(() => ({}))) as { properties?: { nodeResourceGroup?: string } };
...
export async function getAzureCostByService(cloudProviderId: string, resourceGroup?: string): Promise<{ ok: true; services: ServiceCost[] } | { ok: false; error: string }> {
  const tok = await getAzureAccessToken(cloudProviderId);
  if (!tok.ok) return { ok: false, error: tok.error };
  const sub = await subscriptionOf(cloudProviderId);
  if (!sub) return { ok: false, error: "No Azure subscription." };
  const scope = resourceGroup ? `/subscriptions/${sub}/resourceGroups/${resourceGroup}` : `/subscriptions/${sub}`;
  ...
  const data = (await res.json().catch(() => ({}))) as { properties?: { columns?: Array<{ name?: string }>; rows?: unknown[][] }; error?: { message?: string } };
  ...
  const services = (data.properties?.rows ?? [])
    .map((r) => ({ service: String(r[si >= 0 ? si : 1] ?? "Unknown"), cents: Math.round((Number(r[ci >= 0 ? ci : 0]) || 0) * 100) }))
    .filter((s) => s.cents > 0)
...
export function forecastFromMtd(totalCents: number, now: { day: number; daysInMonth: number }): number {
