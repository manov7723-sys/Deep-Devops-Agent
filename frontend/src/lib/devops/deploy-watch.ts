...
type HealthProbe =
  { state: "unreachable" } | { state: "absent" } | { state: "present"; healthy: boolean };
...
async function probe(
  ...
  const res = await listKubernetesResourcesTool.execute(
...
export async function runDeployWatchdog(now: Date): Promise<number> {
  ...
  for (const w of watches) {
    ...
    if (h.state === "absent") {
      await prisma.deployWatch
        .update({ where: { id: w.id }, data: { enabled: false, lastCheckedAt: now } })
        .catch(() => {});
      continue;
    }
    ...
    if (h.healthy) {
      await prisma.deployWatch
        .update({
          where: { id: w.id },
          data: { consecutiveUnhealthy: 0, lastHealthyAt: now, lastCheckedAt: now },
        })
        .catch(() => {});
      continue;
    }
    ...
    const inCooldown =
      w.autoRolledBackAt != null && now.getTime() - w.autoRolledBackAt.getTime() < COOLDOWN_MS;
    if (count < THRESHOLD || inCooldown) {
      await prisma.deployWatch
        .update({ where: { id: w.id }, data: { consecutiveUnhealthy: count, lastCheckedAt: now } })
        .catch(() => {});
      continue;
    }

    // Threshold reached → auto-roll-back.
    const rb = await rollbackDeployment(w.projectId, w.envKey, w.appName, {
    ...
    if (rb.ok) {
      ...
      await prisma.deployWatch
        .update({
          where: { id: w.id },
          data: { consecutiveUnhealthy: 0, autoRolledBackAt: now, lastCheckedAt: now },
        })
        .catch(() => {});
      ...
      await recordDeployment(
        ...
        spec,
        "rolled_back",
        "Watchdog auto-rollback: app was unhealthy for 5+ minutes.",
        "watchdog",
      );
      ...
      await postEventToChatOps(
        w.projectId,
        "🛟",
        `Watchdog rolled back ${w.appName} → ${w.envKey}`,
        note,
      ).catch(() => {});
      ...
      await postEventToChatOps(
        w.projectId,
        "⚠️",
        `Watchdog: ${w.appName} unhealthy, no rollback`,
        note,
      ).catch(() => {});
      await emailProjectMembers(
        w.projectId,
        `⚠️ ${w.appName} unhealthy — no version to roll back to`,
        note,
      ).catch(() => {});
    }
  }
