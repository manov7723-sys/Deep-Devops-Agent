...
export type AppVersions = { app: string; versions: Record<string, { image: string; ready: string }> };
...
export async function getPromotionMatrix(
  projectId: string,
  nsFilter?: string,
): Promise<{ envs: EnvCol[]; apps: AppVersions[]; namespaces: string[]; namespace: string; diag: EnvDiag[]; note?: string }> {
  ...
  const envCols: EnvCol[] = envs.map((e) => ({
    key: e.key, name: e.name, isProduction: e.isProduction, connected: e.hasKubeconfig,
  }));
...
export async function promoteApp(
  ...
  if (!fromEnvKey || !toEnvKey || fromEnvKey === toEnvKey) return { ok: false, error: "Pick a different source and target environment." };
  ...
  if (!source.hasKubeconfig) return { ok: false, error: `Source env "${source.name}" has no cluster connected — connect one on the Connection page first.` };
  ...
  if (!target.hasKubeconfig) return { ok: false, error: `Connect a cluster to "${target.name}" on the Connection page before promoting to it.` };
