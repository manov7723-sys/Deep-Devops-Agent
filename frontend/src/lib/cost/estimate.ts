...
const DEFAULT_HOURLY: Record<Cloud, number> = {
  aws: 0.0416,
  azure: 0.0416,
  gcp: 0.03351,
  proxmox: 0,
...
export const DEFAULT_INSTANCE_TYPE: Record<Cloud, string> = {
  aws: "t3.medium",
  azure: "b2s",
  gcp: "e2-medium",
  proxmox: "custom",
...
const CONTROL_PLANE_LABEL: Record<Cloud, string> = {
  aws: "EKS control plane",
  azure: "AKS control plane (free tier)",
  gcp: "GKE control plane",
  proxmox: "Proxmox (self-hosted)",
};
...
export function estimateInfraCost(spec: EstimateSpec): EstimateResult {
  ...
  const assumptions: string[] = [
    `On-demand US list prices, ${hours} hrs/month, no reserved/savings-plan discounts.`,
  ];
  const notes: string[] = [];
  ...
  if (nodeCount > 0) {
    ...
    if (typeRaw && known == null)
      notes.push(
        `Instance type "${typeRaw}" not in the price table — used a general-purpose rate ($${hourly}/hr). Estimate is approximate.`,
      );
    const label = typeRaw || "general-purpose node";
    lineItems.push({
      label: `${nodeCount} × ${label} (compute)`,
      monthly: round2(nodeCount * hourly * hours),
    });
  }
  ...
  if (spec.managedK8s) {
    ...
    if (cloud === "gcp")
      notes.push(
        "GKE bills one free zonal/Autopilot cluster per billing account — your first cluster may be $0.",
      );
  }
  ...
  if (storageGb > 0) {
    lineItems.push({
      label: `${storageGb} GB block storage`,
      monthly: round2(storageGb * STORAGE_PER_GB[cloud]),
    });
  }
