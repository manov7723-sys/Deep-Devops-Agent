...
export const setupAzureMonitorAlarmsTool: Tool<Input, Output> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      clusterName: {
        type: "string",
        description: "AKS cluster name. Auto-detected from the kubeconfig if omitted.",
      },
      resourceGroup: {
        type: "string",
        description: "AKS resource group. Auto-resolved if omitted.",
      },
    },
    required: [],
  ...
  async execute(input, ctx) {
    ...
    if (cp?.kind !== "azure")
      return {
        ok: false,
        error: "Azure Monitor alarms are AKS/Azure only; this env isn't on Azure.",
      };

    const clusterName = input.clusterName || (await aksClusterFromEnv(env.id));
    if (!clusterName)
      return { ok: false, error: "Couldn't determine the AKS cluster name. Pass clusterName." };
    ...
    return {
      ...
      output: {
        ...
        details: result.alarms.map((a) => ({
          label: AZURE_METRICS[a.key].label,
          ok: a.ok,
          error: a.error,
        })),
      },
