 ...
 */
export const generateTrivyWorkflowTool: Tool<
  Record<string, never>,
  { files: Array<{ path: string; content: string }>; notes: string[] }
> = {
  ...
  async execute() {
    ...
    return {
      ...
      output: {
        files: [file],
        notes: [
          "Scans deps, secrets and misconfigurations on push/PR.",
          "Fails the build on HIGH/CRITICAL findings that have a fix available.",
        ],
      },
    };
  },
