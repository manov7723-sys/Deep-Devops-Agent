...
import { scheduleDeploy, listScheduledDeploys, cancelScheduledDeploy } from "@/lib/devops/scheduled-deploy";

/** Resolve the requested run time from either an ISO timestamp or a relative delay. Must be in the future. */
function resolveRunAt(runAtISO?: string, delayMinutes?: number): { runAt: Date } | { error: string } {
  const now = Date.now();
  ...
  if (runAtISO?.trim()) {
    ...
    if (Number.isNaN(t.getTime())) return { error: `Couldn't parse the time "${runAtISO}". Use an ISO 8601 timestamp or delayMinutes.` };
    if (t.getTime() <= now + 30_000) return { error: "The scheduled time must be in the future." };
    return { runAt: t };
  }
  return { error: "Provide either runAtISO (an ISO 8601 timestamp) or delayMinutes (minutes from now)." };
}
...
export const scheduleDeploymentTool: Tool<ScheduleInput, {
  id: string;
  appName: string;
  envKey: string;
  runAt: string;
  ...
  inputSchema: {
    ...
    properties: {
      ...
      runAtISO: { type: "string", description: "When to run, as an ISO 8601 timestamp (e.g. '2026-07-02T21:00:00'). Use this OR delayMinutes." },
      delayMinutes: { type: "number", description: "Minutes from now to run (e.g. 120 = in two hours). Use this OR runAtISO." },
  ...
  async execute(input, ctx) {
    ...
    const sd = await scheduleDeploy(ctx.projectId, ctx.userId, {
...
export const listScheduledDeploymentsTool: Tool<Record<string, never>, {
  scheduled: Array<{ id: string; appName: string; envKey: string; image: string; runAt: string; status: string; result: string | null }>;
}> = {
...
export const cancelScheduledDeploymentTool: Tool<{ id: string }, { cancelled: boolean }> = {
  ...
  async execute(input, ctx) {
    ...
    if (!ok) return { ok: false, error: "No pending scheduled deployment with that id (it may have already run or been cancelled)." };
