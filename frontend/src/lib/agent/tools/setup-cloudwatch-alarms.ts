...
export const setupCloudWatchAlarmsTool: Tool<Input, Output> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      email: {
        type: "string",
        description: "Email to notify on ALARM (creates an SNS topic + subscription).",
      },
      metrics: {
        type: "array",
      ...
      clusterName: {
        type: "string",
        description: "EKS cluster name. Auto-detected from the kubeconfig if omitted.",
      },
  ...
  async execute(input, ctx) {
    const env = input.envKey
      ? await prisma.env.findFirst({
          ...
          select: { id: true, key: true, region: true, cloudProviderId: true },
        })
      : await prisma.env.findFirst({
    ...
    if (!env.cloudProviderId)
      return { ok: false, error: "This environment has no cloud provider connected." };
    ...
    if (cp?.kind !== "aws")
      return { ok: false, error: "CloudWatch alarms are AWS/EKS only; this env isn't on AWS." };

    const clusterName = input.clusterName || (await eksClusterFromEnv(env.id));
    if (!clusterName)
      return {
        ok: false,
        error: "Couldn't determine the EKS cluster name from the kubeconfig. Pass clusterName.",
      };
