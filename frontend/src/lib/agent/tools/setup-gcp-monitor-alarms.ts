...
type Output = { clusterName: string; project?: string; alarmsCreated: number; emailWired: boolean; details: Array<{ label: string; ok: boolean; error?: string }> };
...
export const setupGcpMonitorAlarmsTool: Tool<Input, Output> = {
  ...
  inputSchema: {
    type: "object",
    properties: {
      envKey: { type: "string", description: 'Env key, e.g. "release". Omit to use the env with a GCP cluster.' },
      email: { type: "string", description: "Email to notify on alert (creates a notification channel)." },
      metrics: { type: "array", items: { type: "string", enum: METRIC_KEYS }, description: `Subset of: ${METRIC_KEYS.join(", ")}. Default all.` },
      clusterName: { type: "string", description: "GKE cluster name. Auto-detected from the kubeconfig if omitted." },
    },
    required: [],
  ...
  async execute(input, ctx) {
    ...
    if (cp?.kind !== "gcp") return { ok: false, error: "GCP Monitoring alarms are GKE/GCP only; this env isn't on GCP." };
    ...
    return {
      ...
      output: {
        ...
        details: result.alarms.map((a) => ({ label: GCP_METRICS[a.key].label, ok: a.ok, error: a.error })),
      },
    };
  },
};
