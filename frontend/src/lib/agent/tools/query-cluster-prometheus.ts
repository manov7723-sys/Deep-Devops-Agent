...
const PRESETS: Array<{ label: string; query: string }> = [
  { label: "Cluster memory % used", query: `100 * (1 - sum(node_memory_MemAvailable_bytes) / sum(node_memory_MemTotal_bytes))` },
  { label: "Cluster CPU % used", query: `100 * (1 - avg(rate(node_cpu_seconds_total{mode="idle"}[5m])))` },
  { label: "Cluster memory used (GiB)", query: `sum(node_memory_MemTotal_bytes - node_memory_MemAvailable_bytes) / 1024^3` },
  { label: "Running pods", query: `count(kube_pod_status_phase{phase="Running"} == 1)` },
  { label: "Pod restarts (1h)", query: `sum(increase(kube_pod_container_status_restarts_total[1h]))` },
  { label: "Ready nodes", query: `count(kube_node_status_condition{condition="Ready",status="true"} == 1)` },
  // Scope to one app by adding a namespace selector, e.g. namespace="myapp-ns":
  { label: "App memory used (GiB) in a namespace", query: `sum(container_memory_working_set_bytes{namespace="NS",container!=""}) / 1024^3` },
];
...
export const queryClusterPrometheusTool: Tool<Input, Output> = {
  ...
  inputSchema: {
    type: "object",
    properties: {
      query: { type: "string", description: "A PromQL expression to evaluate now (instant query)." },
      envKey: { type: "string", description: 'Environment key, e.g. "release". Omit to use the env with a connected cluster.' },
    },
    required: ["query"],
  ...
  async execute(input, ctx) {
    ...
    if (!env) {
      return { ok: false, error: input.envKey ? `Env "${input.envKey}" not found.` : "No environment has a cluster connected." };
    }
    if (!env.kubeconfigRef) {
      return { ok: false, error: `Env "${env.key}" has no cluster connected. Connect it on the Connection tab first.` };
    }

    const res = await queryClusterPrometheus(env.id, input.query);
    if (!res.ok) return { ok: false, error: res.error };
    const series = res.result.slice(0, 50).map((s) => ({ labels: s.metric, value: s.value ? s.value[1] : "" }));
    return { ok: true, output: { envKey: env.key, query: input.query, resultType: res.resultType, series } };
  },
};
