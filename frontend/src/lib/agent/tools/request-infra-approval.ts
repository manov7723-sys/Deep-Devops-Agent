...
export const requestInfraApprovalTool: Tool<Input, Output> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      title: {
        type: "string",
        description: "Short title for the approval, e.g. 'Create EKS cluster prod-eks'.",
      },
      ...
      instanceType: {
        type: "string",
        description: "Node/VM instance type (checked for oversized/GPU + priced).",
      },
      nodeCount: { type: "number", description: "Number of nodes/VMs (for the cost estimate)." },
      managedK8s: {
        type: "boolean",
        description: "True if this provisions a managed K8s cluster (adds control-plane cost).",
      },
      storageGb: { type: "number", description: "Total block storage GB (for the cost estimate)." },
      loadBalancers: {
        type: "number",
        description: "Public load balancers (for the cost estimate).",
      },
      publicBucket: {
        type: "boolean",
        description:
          "True if this creates a PUBLIC object-storage bucket (will be blocked by policy).",
      },
      name: { type: "string", description: "Terraform run label (same as run_terraform 'name')." },
      stack: {
        type: "string",
        description: "Stable logical stack name (same as run_terraform 'stack').",
      },
      files: {
        type: "object",
        description: "Terraform files as a path→content map (same as run_terraform 'files').",
      },
      planSummary: {
        type: "array",
        ...
        items: {
          type: "object",
          properties: {
            change: { type: "string", enum: ["add", "remove", "info"] },
            text: { type: "string" },
          },
  ...
  async execute(input, ctx) {
    ...
    const files: TerraformFile[] = Object.entries(input.files || {}).map(([path, content]) => ({
      path,
