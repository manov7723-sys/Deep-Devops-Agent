...
import { listSecretKeys, setSecret, syncSecretsToCluster, SECRET_KEY_RE, type SecretKeyInfo } from "@/lib/integrations/secrets-store";

export const listAppSecretsTool: Tool<Record<string, never>, { keys: SecretKeyInfo[] }> = {
  name: "list_app_secrets",
  description: "List the KEYS of the app secrets stored for this project (values are never returned). Use before setting or syncing secrets.",
...
export const setAppSecretTool: Tool<{ key: string; value: string }, { key: string; saved: true }> = {
  ...
  inputSchema: {
    type: "object",
    properties: {
      key: { type: "string", description: "Secret key (e.g. DATABASE_URL). Letters, digits, _, . or -; starts with a letter/underscore." },
  ...
  async execute(input, ctx) {
    if (!SECRET_KEY_RE.test(input.key)) return { ok: false, error: "Invalid key. Use letters, digits, _, . or - (starting with a letter/underscore)." };
...
export const syncAppSecretsTool: Tool<{ envKey: string }, { count: number; namespace: string }> = {
  ...
  inputSchema: {
    type: "object",
    properties: { envKey: { type: "string", description: "Env key whose cluster to sync the secrets to." } },
    required: ["envKey"],
    additionalProperties: false,
  },
  async execute(input, ctx) {
    const res = await syncSecretsToCluster({ projectId: ctx.projectId, userId: ctx.userId }, input.envKey);
