...
import { listEnvThresholds, upsertThreshold, resetThreshold, METRIC_KEYS, type MetricKey, type ResolvedThreshold } from "@/lib/observability/thresholds";

async function resolveEnv(projectId: string, envKey?: string): Promise<{ id: string; key: string } | null> {
  return envKey
    ? prisma.env.findFirst({ where: { projectId, key: envKey }, select: { id: true, key: true } })
    : prisma.env.findFirst({ where: { projectId }, orderBy: { promotionRank: "asc" }, select: { id: true, key: true } });
}

export const listAlertThresholdsTool: Tool<{ envKey?: string }, { envKey: string; thresholds: ResolvedThreshold[] }> = {
  ...
  inputSchema: {
    type: "object",
    properties: { envKey: { type: "string", description: "Env key (e.g. 'release'). Omit for the first env." } },
    required: [],
    additionalProperties: false,
  },
  async execute(input, ctx) {
    const env = await resolveEnv(ctx.projectId, input.envKey);
    if (!env) return { ok: false, error: input.envKey ? `Env "${input.envKey}" not found.` : "This project has no environments." };
    return { ok: true, output: { envKey: env.key, thresholds: await listEnvThresholds(env.id) } };
  },
};

export const setAlertThresholdTool: Tool<
  { envKey?: string; metric: MetricKey; percent: number; severity?: "low" | "medium" | "high"; enabled?: boolean },
...
> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      metric: { type: "string", enum: METRIC_KEYS, description: "Which metric: cpu, memory, or disk." },
      percent: { type: "number", description: "Threshold percent (1–100), e.g. 75 = alert when usage exceeds 75%." },
      severity: { type: "string", enum: ["low", "medium", "high"], description: "Alert severity. Default high." },
      enabled: { type: "boolean", description: "Whether this metric's alerting is on. Default true." },
    },
  ...
  async execute(input, ctx) {
    const env = await resolveEnv(ctx.projectId, input.envKey);
    if (!env) return { ok: false, error: input.envKey ? `Env "${input.envKey}" not found.` : "This project has no environments." };
    const row = await upsertThreshold(ctx.projectId, env.id, input.metric, input.percent, input.severity ?? "high", input.enabled ?? true);
    return { ok: true, output: { envKey: env.key, metric: row.metric, percent: row.percent, severity: row.severity, enabled: row.enabled } };
  },
};

export const resetAlertThresholdTool: Tool<{ envKey?: string; metric: MetricKey }, { envKey: string; metric: string }> = {
