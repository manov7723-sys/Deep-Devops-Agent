...
import { listArtifactRegistries, createArtifactRegistry, setupGithubWif } from "@/lib/cloud/gcp-artifact-registry";
import { generateGarWorkflow } from "@/lib/ci/templates";
import type { Tool } from "./types";

async function gcpProviderId(projectId: string): Promise<string | null> {
  const cp = await prisma.cloudProvider.findFirst({ where: { projectId, kind: "gcp" }, select: { id: true } });
  return cp?.id ?? null;
}

/* ── list existing registries ─────────────────────────────────────────────── */
export const listArtifactRegistriesTool: Tool<{ location: string }, { registries: Array<{ name: string; location: string }> }> = {
  ...
  inputSchema: {
    type: "object",
    properties: { location: { type: "string", description: "Artifact Registry location, e.g. us-central1." } },
    required: ["location"],
  ...
  async execute(input, ctx) {
    ...
    return { ok: true, output: { registries: res.data.map((r) => ({ name: r.name, location: r.location })) } };
  },
};

/* ── create a new registry ────────────────────────────────────────────────── */
export const createArtifactRegistryTool: Tool<{ location: string; repository: string }, { name: string; location: string }> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      repository: { type: "string", description: "Repository id (lowercase, hyphens), e.g. the app name." },
    },
...
export const setupGcpGithubWifTool: Tool<{ repoFullName: string }, { workloadIdentityProvider: string; serviceAccount: string }> = {
  ...
  async execute(input, ctx) {
    ...
    if (!res.ok) return { ok: false, error: res.error };
    return { ok: true, output: { workloadIdentityProvider: res.data.workloadIdentityProvider, serviceAccount: res.data.serviceAccount } };
  },
};
...
export const generateGarWorkflowTool: Tool<GarGenInput, { files: Array<{ path: string; content: string }> }> = {
