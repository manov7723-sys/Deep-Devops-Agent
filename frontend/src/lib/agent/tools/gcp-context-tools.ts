  ...
  async execute(_input, ctx) {
    ...
    if (!prov)
      return {
        ok: false,
        error:
          "No GCP account is connected to this project. Connect one with 'Sign in with Google' on the Cloud providers tab.",
      };
    ...
    return {
      ok: true,
      output: { activeProjectId: prov.gcpProject, count: r.projects.length, projects: r.projects },
    };
  },
};
...
export const setGcpContextTool: Tool<SetInput> = {
  ...
  async execute(input, ctx) {
    ...
    if (Object.keys(data).length === 0)
      return { ok: false, error: "Nothing to save — pass gcpProjectId and/or region." };
    const u = await prisma.cloudProvider.update({
      where: { id: cp.id },
      data,
