...
type Output = { stackTitle: string; reasoning: string; files: Array<{ path: string; content: string }>; notes: string[]; hasDockerfile: boolean };
...
export const generateComposeTool: Tool<Input, Output> = {
  ...
  async execute(input, ctx) {
    ...
    return { ok: true, output: { stackTitle: res.stackTitle, reasoning: res.reasoning, files: res.files, notes: res.notes, hasDockerfile: res.hasDockerfile } };
  },
};
