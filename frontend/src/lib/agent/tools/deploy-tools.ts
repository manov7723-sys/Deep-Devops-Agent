...
import {
  ...
  runDeploy,
  deployStatus,
  setEnvKubeconfigSecret,
  type DeployTarget,
} from "@/lib/devops/deploy";
...
export const listRegistryImagesTool: Tool<
  Record<string, never>,
  { cloud: string; images: RegistryImage[]; note?: string }
> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      autoRollback: {
        type: "boolean",
        description:
          "Auto-revert to the previous version if the new rollout doesn't become healthy. Default TRUE — only set false if the user explicitly asks to disable auto-rollback.",
      },
    },
...
> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      branch: {
        type: "string",
        description: "Branch to watch. Defaults to the repo's default branch.",
      },
      timeoutSeconds: {
        type: "number",
        description: "Max seconds to wait this call (10–300, default 240).",
      },
    },
...
export const writeCdFilesTool: Tool<
  WriteCdInput,
  {
    files: string[];
    branch: string;
...
> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      envKey: {
        type: "string",
        description: "Env key whose namespace the app deploys into (from list_deploy_targets).",
      },
      appName: { type: "string", description: "App / resource name (lowercase DNS label)." },
      image: {
        type: "string",
        description: "Full image reference the CI workflow pushes, e.g. 'registry/app:tag'.",
      },
      containerPort: { type: "number", description: "Port the app listens on. Default 8080." },
      replicas: { type: "number", description: "Number of replicas. Default 1." },
      env: {
      ...
      writeWorkflowOnly: {
        type: "boolean",
        description:
          "Write ONLY the deploy workflow (not the manifest) — use when the manifests were created interactively.",
      },
    },
  ...
  async execute(input, ctx) {
    ...
    const files = buildCdFiles(spec, input.manifestPath).filter((f) =>
      input.writeWorkflowOnly ? f.path.includes(".github/") : true,
    );
    // Target branch: the caller's explicit choice, else the repo's DEFAULT
    // branch. Never a generated `deploy/<app>` side branch — those were only
    // ever there to host a PR, and we direct-commit now (see the write loop
    // below). A side branch just hides the pipeline from the CI/CD tab and
    // from every tool that reads the default branch.
    ...
    // DIRECT-COMMIT — no PR. Same policy as deploy_my_app (commitMode='direct')
    // and every provision_* tool: generated CI/CD files land straight on the
    // target branch so the pipeline is immediately runnable and nothing is
    // stranded behind an unmerged PR.
...
export const setKubeconfigSecretTool: Tool<
  { repoFullName: string; envKey: string },
  { secret: string }
> = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      envKey: {
        type: "string",
        description: "Env key whose kubeconfig to publish (from list_deploy_targets).",
      },
    },
...
> = {
  ...
  async execute(input, ctx) {
    ...
    return {
      ok: true,
      output: { found: res.found, ready: res.ready, healthy: res.healthy, pods: res.pods },
    };
  },
};
