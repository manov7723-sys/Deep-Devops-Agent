  ...
  return {
    ...
    panels: [
      panel(
        1,
        "CPU cores",
        `sum(rate(container_cpu_usage_seconds_total{${ns},container!=""}[5m]))`,
        "none",
        0,
        0,
      ),
      panel(
        2,
        "Memory",
        `sum(container_memory_working_set_bytes{${ns},container!=""})`,
        "bytes",
        12,
        0,
      ),
      ...
      panel(
        4,
        "Replicas ready",
        `sum(kube_deployment_status_replicas_ready{${ns}})`,
        "none",
        12,
        8,
      ),
