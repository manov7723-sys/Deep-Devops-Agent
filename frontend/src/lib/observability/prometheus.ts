...
async function promEndpoint(projectId: string): Promise<{ endpoint: string; headers: Record<string, string> } | { error: string }> {
  const creds = await getIntegrationCredentials(projectId, "prometheus");
  if (!creds) return { error: "No Prometheus is connected to this project. Connect one on the Observability page." };
  ...
  if (creds.credentials.bearer_token) headers.Authorization = `Bearer ${creds.credentials.bearer_token}`;
...
async function promFetch(url: string, headers: Record<string, string>): Promise<PromQueryResult> {
  ...
  try {
    ...
    return { ok: false, error: `Couldn't reach Prometheus: ${e instanceof Error ? e.message : "network error"}` };
  }
  ...
  return { ok: true, resultType: body.data?.resultType ?? "vector", result: body.data?.result ?? [], endpoint: "" };
}

/** Instant PromQL query — a single value per series (the "now" snapshot). */
export async function queryPrometheusInstant(projectId: string, query: string): Promise<PromQueryResult> {
...
export const PROM_PRESETS: { key: string; label: string; query: string; unit: string }[] = [
  { key: "cpu", label: "Cluster CPU cores used", unit: "cores", query: `sum(rate(container_cpu_usage_seconds_total{container!=""}[5m]))` },
  { key: "mem", label: "Cluster memory used", unit: "bytes", query: `sum(container_memory_working_set_bytes{container!=""})` },
  { key: "pods", label: "Running pods", unit: "count", query: `count(kube_pod_status_phase{phase="Running"} == 1)` },
  { key: "restarts", label: "Pod restarts (1h)", unit: "count", query: `sum(increase(kube_pod_container_status_restarts_total[1h]))` },
  { key: "nodes", label: "Ready nodes", unit: "count", query: `count(kube_node_status_condition{condition="Ready",status="true"} == 1)` },
];
